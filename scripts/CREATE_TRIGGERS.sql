...
CREATE OR REPLACE TRIGGER TRIGGER_INSERT_PARAMETRO
BEFORE INSERT ON DIV_PARAMETRO_TRANCHE
FOR EACH ROW
BEGIN
	IF :NEW.SEQ_PARAMETRO IS NULL THEN
		SELECT (SEQ_COUNT+1) INTO :NEW.SEQ_PARAMETRO FROM SEQUENCE WHERE seq_name = 'seq_parametro';
		UPDATE SEQUENCE SET SEQ_COUNT = :NEW.SEQ_PARAMETRO WHERE seq_name = 'seq_parametro';
	END IF;
END;
