...
public class Tranche {
	...
	public String dbInsert(Contract contratoCSV, Map<String, Moeda> currenciesAlreadyInserted, Sistema system,
			Garantia garantia, InstituicaoFinanceira instituicaoFinanceira) {
		...
		String campos = "(DAT_INICIAL," + "NOM_TRANCHE_CONTRATO," + "SEQ_CONTRATO, " + "COD_TRANCHE_CONTRATO, " + "SEQ_MOEDA_CONTRATADA, "
				+ "SEQ_MOEDA_LOCAL, " + "SEQ_SISTEMA_AMORT, " + "SEQ_GARANTIA, " + "NUM_CARENCIA_MESES, "
				+ "DSC_JUROS, " + "DSC_CORRECAO, " + "SEQ_INST_FINANCEIRA " + ")";
		...
		values.append(contratoCSV.getContrato() + ",");
		...
		values.append(instituicaoFinanceira.getId());
	...
	public String dbInsertParametro() {
		String insert = "";
		String tabela = "DIV_PARAMETRO_TRANCHE";
		String campos = "(SEQ_TRANCHE_CONTRATO, NUM_DIAS_FECHAMENTO, IND_TIPO_CONTAGEM, IND_TIPO_PAGAMENTO)";
		StringBuilder values = new StringBuilder();
		values.append("" + this.id + ",");
		values.append("" + 0 + ",");
		values.append("'DIAS_UTEIS',");
		values.append("'DIA_UTIL_POSTERIOR'");
		insert = DBInserter.INSERT_INTO + DBInserter.OWNER + "." + tabela + campos + DBInserter.STR_VALUES + values
				+ DBInserter.CLOSING;
		return insert;
	}
